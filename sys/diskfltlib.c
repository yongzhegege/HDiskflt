...
#define SharedUserData ((KUSER_SHARED_DATA * const)0xFFFFF78000000000)
#endif

#ifndef KeQuerySystemTime
#define KeQuerySystemTime(CurrentTime) { \
    ULONG t1, t2; \
    do { \
        t1 = SharedUserData->SystemTime.High1Time; \
        (CurrentTime)->LowPart = SharedUserData->SystemTime.LowPart; \
        t2 = SharedUserData->SystemTime.High2Time; \
    } while (t1 != t2); \
    (CurrentTime)->HighPart = t1; \
}
#endif
...

{
	...
	//创建一个过滤设备对象，设备类型FILE_DEVICE_DISK，没有指定设备名，使用DEVICE_EXTENSION作为设备扩展
...

{
    ...
    // If this is a shutdown IRP, set the global shutdown flag
    if (IRP_MJ_SHUTDOWN == Irp->Tail.Overlay.CurrentStackLocation->MajorFunction)
    {
        g_IsShutdown = TRUE;
        DebugPrint((0, "DiskPerfShutdownFlush: Shutdown detected. Enabling safe mode.\n"));
    }
